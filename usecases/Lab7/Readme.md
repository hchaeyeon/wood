# 계좌 관리 어시스턴트 생성하기
## Action 생성하기
1. **New Action**를 선택하고 액션 이름을 지정합니다.  
   - 액션명 : 
      ```
      새 당좌 예금 계좌를 개설하고 싶습니다
      ```
2. **Customer starts with**를 선택합니다. Action 트리거 문구를 작성하는 창입니다. 사용자가 입력한 문구가 Action을 인식하는지 테스트 한후 인식하지 못하는 문구를 추가로 입력하여 사용자의 문구를 이해하는 훈련을 진행합니다.   
      ```Preview```버튼을 클릭합니다.   
...
2. **New Sept** 버튼을 클릭하여 두번째 스텝을 추가합니다.   
...
3. **New Sept** 버튼을 클릭하여 세번째 스텝을 추가합니다.   
   ...
   - **And then > Continue to next step > Connect to agent** 을 선택합니다.   
      ...
      [Connecting to a live agent 문서](https://cloud.ibm.com/docs/watson-assistant?topic=watson-assistant-human-agent)   
...
4. **New Sept** 버튼을 클릭하여 네번째 스텝을 추가합니다.   
   ...
   - **Assistant says**  
     사용자로 부터 입력 받은 변수를 Assistant가 말하도록 설정합니다.   
   ...
   - **And then > End the action** 을 선택합니다.  
      옵션 상장에서 ```End the action```을 선택합니다.
